...
void Turtlebot3Drive::timer_callback_()
{
  ...
  message.data = std::to_string(left_distance) + " " + std::to_string(TD) + " " + std::to_string(PID) + " " + std::to_string(alpha) ;
...
void Turtlebot3Drive::scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
{
  ...
  if (laser_ranges[15] > 0 ){
    if(laser_ranges[15] > side_min){
      A1 = side_min;
    }
    else{
      A1 = laser_ranges[15];
...
void Turtlebot3Drive::update_callback()
{
  ...
  //RCLCPP_INFO(this->get_logger(), "PID: %lf, alpha: %lf, wall distance: %lf, distance at 90 deg: %lf", pid, alpha, CD, left_distance);
  PID = pid;
  TD = CD;
}
